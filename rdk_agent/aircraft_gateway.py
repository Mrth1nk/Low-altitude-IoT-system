...
def parse_payload(msg_id: int, system_id: int, component_id: int, payload: bytes) -> dict | None:
    try:
        ...
        if msg_id == 1 and len(payload) >= 31:
            ...
            return {"type": "SYS_STATUS", "text": f"电池 {battery}% {voltage:.2f}V"}
        if msg_id == 33 and len(payload) >= 28:
            ...
            return {"type": "GLOBAL_POSITION_INT", "text": f"位置 {lat:.7f}, {lon:.7f} 高 {rel_alt:.1f}m"}
        if msg_id == 74 and len(payload) >= 20:
            ...
            return {"type": "VFR_HUD", "text": f"速度 {ground_speed:.1f}m/s 航向 {heading} 油门 {throttle}%"}
        ...
        if msg_id == 42 and len(payload) >= 2:
            ...
            return {"type": "MISSION_CURRENT", "text": f"当前任务 seq={seq}", "seq": seq}
...
class AircraftGatewayState:
    ...
    def record_packet(self, port: int, data: bytes, remote: tuple[str, int]) -> None:
        ...
        with self.lock:
            ...
            for frame in frames:
                self.messages.append({"time": now, **frame})
